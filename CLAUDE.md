...
│   ├── live/              # ★ OURS: live capture over Web Bluetooth. raceboxTransport/dragyTransport (NUS GATT), ubxRingBuffer (resync on B5 62 + checksum — a notification is not a packet), raceboxDecoder/dragyDecoder (UBX NAV-PVT), dragyHandshake (Dragy sends nothing until handshaked), raceboxSession (stream → ParsedData), liveCapturePackage (★ `.rplive` — the versioned, re-openable save format both live-record components write; registered in datalogParser.ts, plan 0015)
