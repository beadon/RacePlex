...
1. **In-repo first-party plugins** — `src/plugins/<name>/index.ts`, found via
   ...
   `cloud-sync/` is the reference example: Profile-tab panels that sync local
   data to Supabase, built entirely on the panel framework below.
...
the host mounts them. Slots today are `PanelSlot.Coach`, `PanelSlot.Tools`, and
`PanelSlot.Profile`; new slots are just new strings — no registry or framework
changes needed.
...
- The host (`PluginPanelHost`) renders each panel inside a titled card and wraps
  ...
  crashing the tab.
- A coach- or tools-slot panel makes its **tab appear automatically** — the tab
  is self-gating on a contributed panel (`Index.tsx`).
...
|-------|------------------|--------|
| Standard build (CI) | yes | coach installs + loads |
