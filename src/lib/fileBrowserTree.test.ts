...
describe("buildBrowserSessions", () => {
  ...
  it("carries hasVescData/hasBmsData/hasHeartRateData from metadata, defaulting to false", () => {
    const metaMap = new Map<string, FileMetadata>([
      ["a.dove", { fileName: "a.dove", trackName: "OKC", courseName: "CW", sessionStartTime: 1, hasVescData: true, hasBmsData: true, hasHeartRateData: true }],
    ...
    expect(a.hasHeartRateData).toBe(true);
    expect(b.hasVescData).toBe(false);
    expect(b.hasBmsData).toBe(false);
    expect(b.hasHeartRateData).toBe(false);
  });
