/**
 * `.rplx` writer — RacePlex's own phone-GPS log format.
 *
 ...
 * parser (`dovexParser.ts`) reads this byte-for-byte — content-wise, `.rplx`
 * *is* `.dovex`, just under RacePlex's own extension instead of one that
 * traces back to the upstream project this app forked from (see the legacy
 * note below).
 *
 * The one principled content difference: we **do not fabricate** channels the
 * phone can't measure. The device logs `sats,hdop,rpm,accel_*`; the browser
 * exposes none of those, so those columns are simply omitted (the Dove parser
 * treats everything past `timestamp,lat,lng,speed_mph` as optional). We keep
 * the real phone channels: altitude, heading/course, and horizontal accuracy.
 *
 * **Legacy `.dovep` files**: sessions recorded before this rename used the
 * `.dovep` ("Dove phone") extension for byte-identical content. Detection is
 * content-based (`isDovexFormat`), not extension-based, so those files keep
 * opening with no migration needed — this rename only changes what *new*
 * sessions are named.
 */
import { MPS_TO_MPH } from '@/lib/parserUtils';
import type { GpsObservation } from './customGps';

export const RPLX_EXTENSION = 'rplx';
...
/** Session metadata written into the `.rplx` preamble (mirrors `DovexMetadata`). */
export interface RplxSessionMeta {
...
export function formatRplxDatetime(epochMs: number): string {
...
/** Build the device-style filename `YYYYMMDD_HHMM.rplx` from the session start. */
export function buildRplxFileName(startEpochMs: number): string {
  const d = new Date(startEpochMs);
  return (
    `${d.getFullYear()}${pad2(d.getMonth() + 1)}${pad2(d.getDate())}_` +
    `${pad2(d.getHours())}${pad2(d.getMinutes())}.${RPLX_EXTENSION}`
  );
}
...
/**
 * Serialize captured observations + session metadata into an `.rplx` document.
 ...
 */
export function serializeRplx(observations: GpsObservation[], meta: RplxSessionMeta = {}): string {
  const firstTs = observations.length > 0 ? observations[0].fix.timestamp : Date.now();
  const datetime = meta.datetime ?? formatRplxDatetime(firstTs);
...
export function serializeRplxBlob(observations: GpsObservation[], meta?: RplxSessionMeta): Blob {
  return new Blob([serializeRplx(observations, meta)], { type: 'text/csv' });
}
